...
// Layout ini akan membungkus SEMUA halaman di bawah /inventaris/*
// (Contoh: /inventaris/tetap, /inventaris/tetap/detail, /inventaris/tidak-tetap)
export default function InventarisRootLayout({ children }) {
  const theme = useTheme();
  const pathname = usePathname();
  const searchParams = useSearchParams();

  // 1. Mengekstrak 'tipe' dari URL secara dinamis
  // Contoh: /inventaris/tetap/detail -> segments = ['', 'inventaris', 'tetap', 'detail']
  const pathSegments = pathname.split('/');
  const type = pathSegments[1] || ''; // Hasilnya 'tetap' atau 'tidak-tetap'

  // 2. Menentukan logika kondisional berdasarkan 'type' dan 'pathname'
  const isTipeTetap = type === 'inventaris-tetap';

  // Halaman dianggap "list page" (yang butuh filter) jika:
  // - Tipenya 'tetap' DAN path-nya mengandung '/detail'
